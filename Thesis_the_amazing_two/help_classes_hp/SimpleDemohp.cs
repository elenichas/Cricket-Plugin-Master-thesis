...
namespace Thesis.help_classes_hp
{
    ...
    public class SimpleDemohp
    {
        ...
        public SimpleDemohp(Vector3d inSize, Vector3d outSize, List<Voxel> inputVoxels, int pattern_Size, bool probabilistic, bool periodic, List<double> InputPercentages, bool buildingProgram)
        {
            //from this model we get our input
            InputModelhp model = new InputModelhp(new Coord3D((int)inSize.X, (int)inSize.Y, (int)inSize.Z), inputVoxels);
            ...
            Model = new SimpleModelhp(model, pattern_Size, outputSizeInCoord, periodic, false, probabilistic, InputPercentages, buildingProgram);
        }
